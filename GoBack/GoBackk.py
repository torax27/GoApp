...
class BoardStates():
    def __init__(self, move, ids):
        self.move = move
        self.state = self.create_state(ids)


    def create_state(self, ids): #list with [x][y] scheme
        state = [["Go", "Go", "Go", "Go", "Go", "Go", "Go", "Go", "Go"],
        ...
        for x in range(0,9):
            for y in range(0,9):
                state[x][y] = ids["id"+str(x+1)+str(y+1)].text 
...
class Board(Screen):
    ...
    def check_killing(self, groups, current_move): # checks if group gets killed
            ...
            if group[0] != "Edge" and group[0] != "Go": # only check groups that contain stones
                ...
                for stone in group[1:]:
                    ...
                    if stone == group[len(group)-1] and not liberties: # Last stone checked, no liberties found 
                        if group[0] == self.opponent: # enemy 
                            # KO
                            if group[1] == self.previous_move and self.previous_kill[1] == current_move \
                                and len(self.previous_kill)==2 and len(group) == 2:
                                self.illegal_move(current_move)
                                print("Ko")
                                return

                            else:
                                self.remove_stones(group) # group gets removed
                                self.previous_kill = group
                                murder = True
        ...
        self.previous_move = current_move # last piece of code when no illegal move happened
